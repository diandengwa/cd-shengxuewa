...
async def query_district_map(
    ...
    try:
        ...
        if not parsed_address.get("district"):
            logger.info(f"地址未识别到行政区，尝试全局招生范围匹配: {address}")
            matched_schools = []
            keyword = parsed_address.get("community") or parsed_address.get("road")
            if not keyword and len(address.strip()) >= 3:
                keyword = address.strip()
                
            keyword_clean = re.sub(r'^(四川省|成都市|四川|成都)', '', keyword) if keyword else ""
            
            if keyword_clean:
                data = DistrictDataLoader.load_data()
                for dist, dist_data in data.get("districts", {}).items():
                    for school_data in dist_data.get("schools", []):
                        enrollment_range = school_data.get("enrollment_range") or ""
                        if keyword_clean in enrollment_range or keyword_clean in school_data.get("school_name", ""):
                            # 包装成 DistrictSchool 对象并补充 district
                            s_data = dict(school_data)
                            s_data["district"] = dist
                            matched_schools.append(DistrictSchool(**s_data))
            
            if matched_schools:
                # 智能自动补全 district
                parsed_address["district"] = matched_schools[0].district
                logger.info(f"全局匹配成功，自动补全区划为: {parsed_address['district']}")
            else:
                raise HTTPException(
                    status_code=400,
                    detail={
                        "code": 400,
                        "message": "无法识别地址中的行政区划名称，如：天府新区",
                        "data": {
